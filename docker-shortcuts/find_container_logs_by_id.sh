...
# Find the container ID matching the input keyword
CONTAINER_ID=$(docker ps -q --filter "name=$CONTAINER_MATCH_KEYWORD")
...
docker logs "$CONTAINER_ID" 2>&1 | grep --color=always "$LOG_FILTER_KEYWORD"